...
2. ⚠️ 【BTO・完成品PC提案の絶対禁止】
   - BTOパソコン（ドスパラ、パソコン工房、マウス等の完成品PC）の提案は絶対にしないこと
   - 「BTOの方がいい」「完成品の方が安い」等の誘導も禁止
   - ユーザーがBTOを聞いてきても「ここでは自作パーツの提案をしています」と答えること
   - 自作PCパーツの個別提案のみ行うこと
3. ユーザーが言っていない用途を勝手に追加しない（VR、配信など）
4. DBに存在しないスペック値を推測しない（不明なら「不明」）
5. 「少し詳しく教えてください」は絶対に言わない。常に具体的な質問をする
6. 毎回の返答は必ず「具体的な質問」か「具体的な提案」で終える
   ⚠️ 不要な追加質問をしないこと。用途・予算がわかったら即座にsearch_partsで検索。
   「IntelとAMDどちら？」「ATXとMicro-ATXどちら？」等は聞かない。最適な1つを提案する。
7. ⚠️ 【製品DB制約・最重要ルール】
...
10. ⚠️ 【サマリー生成の厳格ルール】
   ...
   - get_build_summaryの結果のsummaryテキストを一字一句そのまま返答に含めること
   - summaryテキストの書き換え・省略・独自テキストの追加は絶対禁止
   - summaryの後に追加コメント（SSD案内等）を付けるのはOK
11. 「データベース」「DB」「在庫」という言葉を使わない。内部の仕組みを説明しない
12. ⚠️ 【カテゴリ名ヘッダー必須】提案メッセージの冒頭に必ず【カテゴリ名】を付けること。省略禁止。
   GPU→【GPU】、CPU→【CPU】、MB→【マザーボード】、RAM→【メモリ】、ケース→【ケース】、電源→【電源】
   例: 「【マザーボード】14700Kに合わせて、MSI MAG B760 TOMAHAWK WIFI（¥28,291）をおすすめします。」
13. ⚠️ 【価格正確性ルール】提案する製品の価格はsearch_partsの結果に含まれるprice_minをそのまま使うこと。
   自分の知識で価格を推測・変更するのは絶対禁止。DBの価格と異なる金額を書いてはいけない。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ うちの店の品揃え（価格は変動するが、目安として把握しておくこと）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

| カテゴリ | 件数 | 実用価格帯 | 中央値 | 備考 |
|---|---|---|---|---|
| GPU（現行世代）| ~200 | ¥37,000〜¥200,000 | ¥119,000 | RTX 40/50, RX 7/9, Arc B |
| CPU | ~120 | ¥23,000〜¥97,000 | ¥55,000 | Intel 12-14世代, Ryzen 7000/9000 |
| マザーボード | ~3,000 | ¥17,000〜¥184,000 | ¥55,000 | 各ソケット対応 |
| メモリ | ~2,500 | ¥7,000〜¥190,000 | ¥39,000 | 2枚組キット~1,400件 |
| ケース | ~1,200 | ¥8,000〜¥35,000 | ¥14,000 | ATX/MicroATX/Mini-ITX |
...
★ 品揃えテーブルと下記おすすめラインを参考に、用途に応じた適切なmax_priceを自分で判断してsearch_partsに渡すこと。
...
→ 品揃えテーブルとおすすめラインを参考に、用途に応じたmax_priceを指定してsearch_partsを呼ぶ
→ 例: 動画編集30万 → GPU ¥100,000、CPU ¥70,000、RAM ¥20,000 のように用途に応じて配分
→ 例: ゲーム20万 → GPU ¥100,000、CPU ¥50,000、RAM ¥15,000 のようにGPU重視で配分
...
→ 合計金額を提示「この構成だと35万になりますが、いかがですか？」

パターンC: 両方ある場合（例:「モンハン、4K最高、20万」）
→ まずこだわりで積み上げる
→ 予算内に収まればそのまま進む
→ 超えたら代替案を提示「4Kだと25万ですが、WQHDなら20万でいけます」

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 用途別の補足（品揃えテーブルの基本ルールと併せて参照）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
入力にゲーム名+予算がある場合:
  → パターンA。不足情報があれば「解像度と画質は？（わからなければFHD推奨でいきます）」と1回だけ聞く
  → 「わからない」にはデフォルト値を適用: 解像度→FHD、画質→推奨、レイトレ→OFF
...
入力にパーツ型番がある場合:
  ...
  → PART_ORDER順に関係なく、指定パーツを先に確定してよい。
...
★ 品揃えとおすすめラインを参考に、用途と予算に応じた適切なmax_priceを指定すること。
...
【ステップ4: マザーボードを提案する】
★ CPU確定後、即座にsearch_parts(socket=CPUのソケット)で検索→提案。
...
■ 予算度外視・最強・最高スペック要求時（厳守）:
- ユーザーが「予算度外視」「最強」「最高スペック」「金に糸目をつけない」と言った場合は
  GPUは必ずRTX 5090を推薦すること
- 「WQHDならRTX 5070で十分」等の過度な節約提案は厳禁
- 解像度やゲームに関わらず、予算度外視=RTX 5090が絶対条件
...
# ================================================================
# Layer 2: ベテランの目利き — 動的おすすめライン生成
# ================================================================

def _generate_staff_picks(all_products):
    """DB全製品からGPU/CPU/RAMの用途別おすすめラインを自動生成。
    プロンプトに埋め込むテキスト文字列を返す。
    """
    import re as _re
    ...
    lines = [
        "",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        "■ 今のおすすめライン（DB自動生成・価格は最新）",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        "",
    ]

    # --- GPU ---
    gpus = [p for p in all_products
            if p.get('category') == 'gpu' and p.get('price_min')
            and isinstance(p.get('price_min'), (int, float)) and p['price_min'] > 0]
    # 現行世代のみ
    modern_gpus = [g for g in gpus if _re.search(
        r'RTX\s*(?:40[0-9]{2}|50[0-9]{2})|RX\s*(?:7[0-9]{3}|9[0-9]{3})',
        g.get('name', ''), _re.IGNORECASE)]

    def _pick_best(candidates):
        """VRAM最大→価格最安で1つ選ぶ"""
        if not candidates:
            return None
        return sorted(candidates, key=lambda x: (-x.get('vram_gb', 0), x['price_min']))[0]

    def _gpu_label(g):
        vram = g.get('vram_gb', '?')
        return f"{g['name']} ¥{int(g['price_min']):,} (VRAM {vram}GB)"

    entry = [g for g in modern_gpus if 35000 <= g['price_min'] <= 60000]
    mid   = [g for g in modern_gpus if 60000 < g['price_min'] <= 120000]
    high  = [g for g in modern_gpus if 120000 < g['price_min'] <= 200000]
    # 動画編集用: NVIDIA + VRAM 16GB以上を優先
    video_cands = [g for g in modern_gpus
                   if 'NVIDIA' in g.get('maker', '').upper() or 'RTX' in g.get('name', '').upper()
                   if g.get('vram_gb', 0) >= 16 and 70000 <= g['price_min'] <= 150000]
    if not video_cands:
        video_cands = [g for g in modern_gpus
                       if 'RTX' in g.get('name', '').upper()
                       and 70000 <= g['price_min'] <= 150000]

    lines.append("【GPU】")
    pick_entry = _pick_best(entry)
    pick_mid   = _pick_best(mid)
    pick_high  = _pick_best(high)
    pick_video = _pick_best(video_cands)

    if pick_entry:
        lines.append(f"  ゲーム入門:  {_gpu_label(pick_entry)}")
    if pick_mid:
        lines.append(f"  ゲーム中級:  {_gpu_label(pick_mid)}")
    if pick_video and pick_video != pick_mid:
        lines.append(f"  動画編集:    {_gpu_label(pick_video)}")
    if pick_high:
        lines.append(f"  ゲーム上級:  {_gpu_label(pick_high)}")

    # --- CPU ---
    cpus = [p for p in all_products
            if p.get('category') == 'cpu' and p.get('price_min')
            and isinstance(p.get('price_min'), (int, float)) and p['price_min'] > 0]

    def _cpu_label(c):
        cores = c.get('cores', '?')
        threads = c.get('threads', '?')
        return f"{c['name']} ¥{int(c['price_min']):,} ({cores}C{threads}T)"

    # コスパ: 最安帯で6コア以上
    cospa_cpus = [c for c in cpus if c.get('cores', 0) >= 6 and 20000 <= c['price_min'] <= 40000]
    cospa_cpus.sort(key=lambda x: x['price_min'])

    # ゲーム: 8コア帯
    game_cpus = [c for c in cpus if c.get('cores', 0) >= 8 and 40000 < c['price_min'] <= 60000]
    game_cpus.sort(key=lambda x: (-x.get('cores', 0), x['price_min']))

    # 動画編集: コア数重視
    video_cpus = [c for c in cpus if c.get('cores', 0) >= 12 and 50000 < c['price_min'] <= 100000]
    video_cpus.sort(key=lambda x: (-x.get('cores', 0), x['price_min']))

    lines.append("【CPU】")
    if cospa_cpus:
        lines.append(f"  コスパ:  {_cpu_label(cospa_cpus[0])}")
    if game_cpus:
        lines.append(f"  ゲーム:  {_cpu_label(game_cpus[0])}")
    if video_cpus:
        lines.append(f"  動画:    {_cpu_label(video_cpus[0])}")

    # --- RAM (DDR5 32GBキット) ---
    rams = [p for p in all_products
            if p.get('category') == 'ram' and p.get('price_min')
            and isinstance(p.get('price_min'), (int, float)) and p['price_min'] > 0]

    # 2枚組 32GB DDR5 キット
    kit_32 = []
    for r in rams:
        name_upper = r.get('name', '').upper()
        # DDR5判定
        if 'DDR5' not in name_upper and r.get('memory_type', '').upper() != 'DDR5':
            continue
        # 2枚組判定（名前に×2, x2, 2枚, kit等）
        if not _re.search(r'[×xX]\s*2|2\s*枚|KIT|2PACK|DUAL', name_upper):
            # capacity_gbが32なら2枚組キットの可能性が高い
            if r.get('capacity_gb', 0) != 32:
                continue
        # 合計32GB判定
        cap = r.get('capacity_gb', 0)
        if cap not in (32, 16):  # 32=キット合計, 16=1枚16GB
            # 名前から判定
            if '32GB' not in name_upper and '16GB' not in name_upper:
                continue
        kit_32.append(r)

    kit_32.sort(key=lambda x: x['price_min'])

    lines.append("【RAM 32GBキット（DDR5）】")
    if kit_32:
        lines.append(f"  最安:    {kit_32[0]['name']} ¥{int(kit_32[0]['price_min']):,}")
        # 高速: DDR5-6000以上で最安
        fast = [k for k in kit_32 if _re.search(r'(?:6[0-9]{3}|7[0-9]{3})', k.get('name', ''))]
        if fast:
            lines.append(f"  高速:    {fast[0]['name']} ¥{int(fast[0]['price_min']):,}")
    else:
        lines.append("  （DDR5 32GBキットデータなし — search_partsで検索してください）")

    lines.append("")
    lines.append("★ 上記は目安。実際の提案はsearch_partsで検索した結果のみ使うこと。")
...
SEARCH_PARTS_TOOL = {
    ...
    "input_schema": {
        ...
        "properties": {
            ...
            "max_price": {
                "type": "integer",
                "description": "最大価格（円）。品揃えとおすすめラインを参考に適切な額を指定すること"
            },
            "min_vram_gb": {
                "type": "integer",
...
def handle_search_parts(params, all_products, session=None, limit=5, _internal=False):
    ...
    return {
        ...
        'count': len(results),
        'instruction': (
            '【カテゴリ名】ヘッダー必須。価格はprice_minをそのまま引用。'
...
def _handle_search_bto(params, session):
    ...
    for tier_key in ['value', 'recommended', 'premium']:
        ...
        tiers.append({
            ...
            'ram_type': rec.get('ram_type', ''),
            'ram_speed_mhz': rec.get('ram_speed_mhz', 0),
            'storage': rec.get('storage', ''),
            'psu': rec.get('psu', ''),
            'case_form_factor': rec.get('case_form_factor', ''),
...
def generate_server_side_summary(session):
    ...
    import re as _re
    def _extract_ddr(s):
        """'DDR5 SDRAM' → 'DDR5', 'DDR4' → 'DDR4'"""
        m = _re.search(r'DDR\d+', str(s).upper())
        return m.group(0) if m else ''

    ram_part = build.get('ram') or {}
    ram_type_raw = ram_part.get('memory_type', '')
    ram_type_str = ram_type_raw[0] if isinstance(ram_type_raw, list) else str(ram_type_raw)
    ram_ddr = _extract_ddr(ram_type_str)

    mb_mem_raw = mb_part.get('memory_type', '')
    mb_mem_list = mb_mem_raw if isinstance(mb_mem_raw, list) else ([str(mb_mem_raw)] if mb_mem_raw else [])
    mb_ddrs = [_extract_ddr(t) for t in mb_mem_list]
    mb_ddrs = [d for d in mb_ddrs if d]

    if ram_ddr and mb_ddrs:
        if ram_ddr in mb_ddrs:
            compat_lines.append(f"✅ メモリ規格: {ram_ddr} 一致")
        else:
            compat_lines.append(f"❌ メモリ規格: {ram_ddr} vs {', '.join(mb_ddrs)} 不一致")
...
def _call_claude_with_tools_gen(session, user_message, all_products, system_prompt):
    ...
    for iteration in range(max_iterations):
        ...
        except req_lib.exceptions.HTTPError as api_err:
            err_body = api_err.read().decode('utf-8', errors='replace')
            err_body = api_err.response.text if api_err.response else str(api_err)
            status_code = api_err.response.status_code if api_err.response else 0
            print(f"[CLAUDE_API_ERROR] HTTP {status_code}: {err_body[:500]}", flush=True, file=sys.stderr)
            try:
                err_json = json.loads(err_body)
                err_msg = err_json.get('error', {}).get('message', err_body[:200])
            except Exception:
                err_msg = err_body[:200]
            raise RuntimeError(f"AI API エラー (HTTP {status_code}): {err_msg}")
...
def chat():
    ...
    try:
        ...
        if session.get('bto_mode'):
            ...
            # Layer 2: ベテランの目利き（動的おすすめライン）
            staff_picks_text = _generate_staff_picks(all_products)
            system_prompt = (
